...
When docs describe deployment or operations, cross-reference against the real setup: `docker-compose.yml` and `backend/Dockerfile` for local/container config, the Render deployment for the backend, Vercel for the frontend, `.github/workflows/ci.yml` for CI steps, and the health/readiness endpoints exposed by `backend/src/server.ts`. Flag docs that describe a deploy step, env var, or health check that no longer matches these files.
